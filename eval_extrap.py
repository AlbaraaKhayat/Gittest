...
for i in tqdm(range(start,len(observation[0]))):
    for z in range(sequences):
        ...
        xmse_p[z,i-1]=np.mean((observation[z,start-1]-observation[z,i])**2)
        xmae_p[z,i-1]=np.mean(np.abs(observation[z,i]-observation[z,start-1]))
        xssim_p[z,i-1]=evaluu.compare_ssim(observation[z,i],observation[z,start-1],win_size=3,multichannel=True)
        xnse_p[z,i-1]=1-(np.sum((observation[z,start-1]-observation[z,i])**2)/np.sum((observation[z,i]-np.mean(observation[z,i]))**2))
        xstd_p[z,i-1]=np.std(observation[z,start-1])
        xrmsd_p[z,i-1]=np.sqrt(np.sum(np.square(observation[z,start-1]-observation[z,i]))/area)
